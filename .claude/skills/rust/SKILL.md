...
- Library logic in `packages/contextractor_engine/src/`; binary entry point in `apps/contextractor/src/main.rs`
- Re-export the public API from `lib.rs` so call sites import from the crate root

### Environment Variables

- Consolidate environment access in `env.rs` with one function per variable (e.g. `apify_token() -> Result<String>`, `proxy_url() -> Option<String>`)
