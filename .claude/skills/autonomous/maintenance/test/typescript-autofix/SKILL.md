---
name: autonomous:maintenance:test:typescript-autofix
description: WHEN reviewing TypeScript source files for type safety violations and auto-fixing what can be safely resolved. WHEN-NOT for test files or changes to public API signatures.
...
Review `.ts` files under `apps/`, `packages/`, and `tools/`. Exclude `node_modules/`, `dist/`, `target/`, and `*.test.ts` files from manual review (tests are reviewed by the test commands).
