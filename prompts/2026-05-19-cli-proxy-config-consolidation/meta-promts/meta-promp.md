...
## New suggestions: 
- should the repeateble `--proxy <url>` serve for both proxy tier and for the original `--proxy` plus switch together?
- Would that be possible to mirror JSON config files and the CLI switches?
- Review / apply similar improvements and consolidation to all the CLI commands, not only `exctract` (if applicable)
