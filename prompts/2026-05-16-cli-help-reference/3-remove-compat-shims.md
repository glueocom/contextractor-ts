...
Items 1 and 2 (YAML, root shorthand) are independent and can run in any order. Item 3 (`normalizeConfigKeys`) depends on `2-optimize-cli-args.md` being applied first — it removes the `trafilaturaConfig` call sites that are the only callers.
...
**Prerequisite**: Apply `2-optimize-cli-args.md` first. It removes `trafilaturaConfig` from both apps' schemas and configs, eliminating the only callers of `normalizeConfigKeys`.
...
- `apps/standalone/src/config.ts:146` — removed by `2-optimize-cli-args.md`
- `apps/apify-actor/src/config.ts:31` — removed by `2-optimize-cli-args.md`
...
Verify first that `2-optimize-cli-args.md` has been applied:

```bash
grep -rn 'normalizeConfigKeys' apps/
```

Must return no matches before proceeding. If matches remain, apply `2-optimize-cli-args.md` first.
