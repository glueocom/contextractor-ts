...
- Napi prebuilds + lint/test workflow — also add `ci.yml` running `npm run build -ws --if-present / npm run test -ws --if-present`, `cargo test`, `biome check`.
...
- This prompt creates `.github/workflows/build-napi.yml` with a matrix covering `darwin-arm64`, `darwin-x64`, `linux-x64-gnu`, `linux-arm64-gnu`. Pattern follows `napi-rs/package-template-npm`.
...
- Local dev still works without CI: `npm run build -w @contextractor/engine-native` produces the host platform's `.node` for development. Cross-platform prebuilds are only required before deploy.
