...
- Main package `packages/contextractor-engine/native/package.json` lists each `@contextractor/engine-native-<platform>` in `optionalDependencies` with `"*"`. npm picks the matching prebuild via `os` + `cpu` resolution at install time.
...
- The Apify Dockerfile uses `npm run deploy --prod -w @contextractor/apify -- /deploy` — npm deploy resolves the workspace prebuild for the image's arch and produces a self-contained `node_modules` with no symlinks.
