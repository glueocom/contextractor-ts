...
- `pnpm-workspace.yaml` includes `apps/*`, `packages/*`, `packages/*/native`, `packages/*/native/npm/*`, `tools/*`.
- Root `package.json` declares `"private": true`, `packageManager: "pnpm@10..."`, scripts for `build/test/lint/format/check` fanning out via `pnpm -r`.
...
- Apify CLI ≥ 1.4 (`apify --version`), Node 22+, pnpm 10+ — surface a clear error if missing.

## Auto-fix examples

- Missing dev dep → `pnpm add -wD <pkg>`.
