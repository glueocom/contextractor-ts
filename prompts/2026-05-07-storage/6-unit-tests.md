...
- `apps/standalone/src/` — CLI argument parsing, format validation (`validateSaveFormats`), config merging, storage integration, sink routing, `original` format handling. Check `sinks.ts` (`createCliSink`, `jsonlSink`, `originalSink`, stdout/ndjson paths, dataset error isolation).
