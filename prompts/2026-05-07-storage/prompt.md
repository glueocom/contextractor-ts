...
```
contextractor extract <url> [<url>…]   [-o, --dataset <name>] [--no-stdout] [--format json|md|html]
...
### Acceptance criteria (verify before marking done)

- [ ] `npm test` passes; lint is clean; types check.
...
- [ ] Two parallel `contextractor extract` runs against different URLs both succeed and produce indexes 1 and 2 with no race-condition data loss.
- [ ] `contextractor serve` on the npm package binds to 127.0.0.1 only; `--host 0.0.0.0` is rejected with a clear message.
- [ ] In Docker, `serve --host 0.0.0.0` without `CONTEXTRACTOR_API_TOKEN` refuses to start.
- [ ] In Docker, `serve --host 0.0.0.0` with the token, requests without `Authorization: Bearer` get 401; `/healthz` does not.
- [ ] `GET /v2/datasets/default/items` returns the Apify-shaped envelope.
- [ ] `GET /v2/datasets/default/items?format=jsonl` returns NDJSON with `Content-Type: application/x-ndjson`.
- [ ] `apify-client` (JS or Python), pointed at `http://localhost:8080`, can list datasets and read items without modification.
- [ ] `docker run --rm contextractor https://example.com` prints JSON to stdout (no `-v` required); same byte-for-byte as the npm version.
...
- Anything that didn't fit and was deferred (with rationale).
- A list of follow-up issues to file (v2 items, found-during-implementation TODOs).
