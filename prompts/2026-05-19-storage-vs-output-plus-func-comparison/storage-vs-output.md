...
The schema default for `saveDestination` remains `key-value-store`. Do not change it. Both `dataset` and `key-value-store` remain valid destinations after this change.
