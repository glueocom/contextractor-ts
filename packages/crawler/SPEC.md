...
- `deduplication?: 'none' | 'url' | 'content-hash'` (default `'url'`) — controls post-fetch deduplication layered on top of Crawlee's built-in URL dedup. `createContextractorCrawler` initialises shared `seenCanonicals: Set<string>` and `seenContentHashes: Set<string>` and passes them to all three handler factories. `'none'`: no additional dedup beyond Crawlee URL dedup. `'url'`: skips pages whose `<link rel="canonical">` was already seen and differs from the current URL; applies to all three handler types. `'content-hash'`: additionally skips pages whose extracted text content hash was already seen.
