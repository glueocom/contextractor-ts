...
- **Zod schema ⇄ generated `INPUT_SCHEMA.json`** — the `@contextractor/schema` snapshot test (`packages/contextractor-schema/test/to-apify-schema.test.ts`) guards zero diff between `toApifyInputSchema(ContextractorInput)` and `apps/contextractor-apify/.actor/input_schema.json`. Run it via `npm run test -w @contextractor/schema`. If it fails, regenerate via `npm run start -w @contextractor/gen-input-schema` and commit the result; never hand-edit the JSON.
...
- **`input_schema.json` drifted from the Zod schema** → re-run `npm run start -w @contextractor/gen-input-schema` and commit the regenerated file. Never hand-edit `input_schema.json`. If the snapshot test still fails, the fix belongs in `packages/contextractor-schema/src/input.ts`.
